/**
 * qrcode.min.js - Vollstaendiger QR Code Generator (offline)
 * API: kompatibel mit qrcode@1.5.x (QRCode.toCanvas)
 * Lizenz: MIT (basierend auf kazuhikoarase/qrcode-generator)
 * DkZ R3: Kein CDN noetig - 100% offline
 */
var QRCode = (function () {
  'use strict';

  // --- QR Code Generator Core (kompakte Implementierung) ---
  ...
  QR8bitByte.prototype = {
    ...
    write: function (buffer) {
      for (var i = 0; i < this.data.length; i++) {
        ...
        if (c > 0xff) {
          // UTF-8 Kodierung fuer Unicode-Zeichen
  ...
  // --- API: QRCode.toCanvas (kompatibel mit qrcode@1.5.x) ---
  function toCanvas(canvas, text, options, cb) {
    if (typeof options === 'function') { cb = options; options = {}; }
    if (!options) options = {};

    try {
      var ecLevel = options.errorCorrectionLevel || 'M';
      var ecMap = { L: QRErrorCorrectLevel.L, M: QRErrorCorrectLevel.M, Q: QRErrorCorrectLevel.Q, H: QRErrorCorrectLevel.H };
      var errorLevel = ecMap[ecLevel] !== undefined ? ecMap[ecLevel] : QRErrorCorrectLevel.M;

      // Auto-detect type number
      var qr = null;
      for (var type = 1; type <= 40; type++) {
        try {
          qr = new QRCodeModel(type, errorLevel);
          qr.addData(text);
          qr.make();
          break;
        } catch (e) {
          // Data too large, try next type
        }
      }
      ...
      var darkColor = colors.dark || '#000000';
      var lightColor = colors.light || '#ffffff';
